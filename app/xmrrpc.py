...
class Wallet_cl(object):
    ...
    def subbolenz(self, idx):
        body = (
            '{"jsonrpc":"2.0","id":"0","method":"get_balance","params":{"account_index":0,"address_indices":[0,'
            + str(int(idx))
            + "]}}"
        )
        resp = requests.post(self.url, data=body, auth=HTTPDigestAuth("test", "test"))
    ...
    def sweepsub(self, idx, adr):
        body = (
            '{"jsonrpc":"2.0","id":"0","method":"sweep_all","params":{"address":"'
            + str(adr)
            + '","subaddr_indices":['
            + str(int(idx))
            + '],"ring_size":11,"unlock_time":0,"get_tx_keys":true}}'
        )
        resp = requests.post(self.url, data=body, auth=HTTPDigestAuth("test", "test"))
        a = resp.json()
        try:
